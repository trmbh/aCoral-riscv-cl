...
//  * @brief 图像预处理
//  */
// static void preprocess_image(const uint8_t* input, uint8_t* output)
// {
//     printf("\n[预处理] 开始处理图像数据...\n");
    
//     // 1. 归一化处理：将uint8转换为float32并除以255
//     printf("[预处理] 转换数据类型并归一化: uint8 -> float32, 除以255\n");
//     float* float_buf = (float*)output;
//     for (int i = 0; i < IMAGE_SIZE * IMAGE_SIZE; i++) {
//         float_buf[i] = input[i] / 255.0f;
//     }

//     // 2. 打印预处理后的数据统计
//     float min_val = float_buf[0];
//     float max_val = float_buf[0];
//     float sum_val = 0;
//     int non_zero_count = 0;
    
//     for (int i = 0; i < IMAGE_SIZE * IMAGE_SIZE; i++) {
//         if (float_buf[i] < min_val) min_val = float_buf[i];
//         if (float_buf[i] > max_val) max_val = float_buf[i];
//         sum_val += float_buf[i];
//         if (float_buf[i] > 0) non_zero_count++;
//     }
...
//         // 预处理图像
//         preprocess_image(image_buf, (uint8_t*)preprocessed_buf);
